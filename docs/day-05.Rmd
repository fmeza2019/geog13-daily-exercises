...
```{r, eval = TRUE}
covid %>% 
  select(county,deaths,cases,date) %>% 
...
```{r, eval = TRUE}
covid %>% 
  select(state,deaths,cases,date) %>% 
  filter(cases > 0, state !="unknown", date == max(date)) %>% 
  group_by(state) %>% 
  summarize(totcases = sum(cases), totdeaths = sum(deaths), ratio = 100 * totdeaths/totcases) %>% 
  slice_max(ratio, n = 5)
```
